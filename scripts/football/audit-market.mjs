...
const initialIds=new Set(populationPlayers(s).filter(p=>p.unit==='senior').map(p=>p.id));
const write=()=>fs.writeFileSync(path,JSON.stringify({scope:'Personnel, growth and market audit without match results or match minutes; league membership held fixed. Verifies economic and registration invariants, not competitive ability balance.',years,hashes,seconds:(performance.now()-start)/1000,complete:rows.length===years*2,rows,failures},null,2)+'\n');
...
write();if(failures.length)process.exitCode=1;
