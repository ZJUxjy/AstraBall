...
import {championshipName,isMetroLeague} from './catalog.js';
import {wageBill} from './market.js';
import {ensureEconomy} from './market.js';
...
const club=id=>clubIdentity(id),name=id=>club(id)?.name||'待定';
const goalName=goal=>isMetroLeague(goal.division)&&goal.target===4?'进入星冠季后赛':goal.label;
const md=date=>`${Number(date.slice(5,7))} 月 ${Number(date.slice(8))} 日`;
const competition=id=>getDivision(id)?.name||leagueSystems.find(s=>s.cup?.id===id)?.cup.name||championshipName(id);
...
function selection(s){
 ...
 <section class="manager-panel manager-club"><span class="manager-kicker">${esc(getDivision(division).name)}</span><h2>${esc(t.name)}</h2><p>${esc(cities.find(x=>x.id===c.city)?.name||'')} · ${c.founded} 年成立</p><div class="manager-stats"><div><strong>${t.roster.length}</strong><span>一线队球员</span></div><div><strong>${Math.round(averageQuality(t))}</strong><span>首发平均能力</span></div></div><div class="manager-objective"><span>赛季目标</span><strong>${esc(goalName(goal))}</strong></div><div class="manager-actions"><button class="football-primary" data-manager="appoint" ${busy?'disabled':''}>接手${esc(t.name)}</button><a class="football-link" href="#squad/${t.id}">查看阵容 ↗</a></div></section></div>`;
}
...
function dashboard(s){
 ...
 const economy=ensureEconomy(s),expiring=clubPlayers(s,own,{unit:'all'}).filter(p=>economy.contracts[p.id]?.end<=dateOf(s.year,12,31));
 const account=economy.accounts[own],bill=wageBill(s,own);
 const callups=t.roster.filter(p=>p.academyCallup);
 const urgent=[...(account.cash<0?[{href:'#finance',text:`资金缺口 ${formatMoney(-account.cash)} 星元 · 查看账本`}]:[]),...(callups.length?[{href:'#youth',text:`${callups.length} 名青训临时进入比赛名单`}]:[]),...issues.slice(0,3).map(p=>({href:`#squad/${own}/${p.id}`,text:`${p.name} · ${p.injuryDays?'伤停 '+p.injuryDays+' 天':p.suspended?'停赛 '+p.suspended+' 场':'体能 '+Math.round(p.condition)+'%'}`})),...(expiring.length?[{href:'#market',text:`${expiring.length} 人本年合同到期 · 查看续约`}]:[]),...(account.review?.restricted?[{href:'#finance',text:'财政审查限制扩大工资 · 查看账本'}]:bill>account.wageLimit?[{href:'#finance',text:'工资超过软线 · 查看超额税'}]:[])];
 const final=Boolean(s.summary),goalStatus=row.played?(met?(final?'已达成':'当前达标'):(final?'未达成':'尚未达标')):'尚未开赛';
 return `<header class="football-title"><div><small>${s.year} 赛季 · ${esc(getDivision(d).name)} · ${md(s.date)}</small><h1>${esc(name(own))}</h1></div><div class="manager-actions"><a class="football-link" href="#squad/${own}/overview">俱乐部总览</a><a class="football-link" href="#match/calendar/all/${own}">俱乐部日历</a><a class="football-link" href="#squad/${own}">球队阵容</a></div></header>
 ...
 <section class="manager-panel manager-priorities"><h2>优先处理</h2>${urgent.length?`<ul>${urgent.map(item=>`<li><a href="${item.href}">${esc(item.text)} ↗</a></li>`).join('')}</ul>`:'<p>暂无紧急事项</p>'}<a class="football-link" href="#finance">现金 ${formatMoney(account.cash)} 星元 · 财政 ↗</a>　<a class="football-link" href="#history">历史战绩 ↗</a></section>
 ...
 <section class="manager-panel"><span class="manager-kicker">赛季目标</span><h2>${esc(goalName(goal))}</h2><p class="manager-goal-status">${goalStatus}</p><div class="manager-objective"><span>执教起始</span><strong>${Number(s.manager.appointed.slice(0,4))} 年 ${md(s.manager.appointed)}</strong></div>${final?`<a class="football-link" href="#match/summary/all">赛季结果 ↗</a>`:`<p>${row.played} / ${getDivision(d).rounds} 轮已完成</p>`}</section>
 <section class="manager-panel"><h2>阵容待办 <small>${issues.length}</small></h2>${issues.length?`<ul class="manager-issues">${issues.map(p=>`<li><a href="#squad/${own}/${p.id}">${esc(p.name)}</a><span>${p.injuryDays?`伤停 ${p.injuryDays} 天`:p.suspended?`停赛 ${p.suspended} 场`:p.playedToday?'今日已出场':`体能 ${Math.round(p.condition)}%`}</span></li>`).join('')}</ul>`:'<p>全员可出场</p>'}<a class="football-link" href="#squad/${own}">查看球员状态 ↗</a></section>
 ...
 <section class="manager-panel"><h2>合同待办 <small>${expiring.length}</small></h2><p>${expiring.length?`${expiring.length} 人的合同将在本年到期`:'本年无到期合同'}</p><a class="football-link" href="#market">管理合同 ↗</a></section>
 ...
 <section class="manager-panel"><h2>青训培养</h2><p>青年比赛、提拔与外租</p><a class="football-link" href="#youth">青训中心 ↗</a><p>皇家学院与年度选秀</p><a class="football-link" href="#youth/draft">查看选秀去向 ↗</a></section>
...
function draw(){if(!root)return;const s=peekSeason();root.innerHTML=(s.manager?dashboard(s):selection(s))+`<p class="manager-status" role="status" aria-live="polite">${esc(message||'本地自动存档')}</p>`;
 const date=document.querySelector('.app-topbar .date');if(date)date.innerHTML=`<span>殖民历</span> ${s.year} <span>年 ${md(s.date)}</span>`;
}
