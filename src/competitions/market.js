...
export function mustOffloadClub(s,id){
 if(!s.economy?.accounts[id])return false;
...
function transferEligibility(s,id,buyer,years=3,{sellerApproved=false,context}={}){
 ...
 if(!competitiveMarket(s)&&currentSkill(s,p)>clubLevel(s,buyer)+11&&!sellerApproved&&!(p.club&&mustOffloadClub(s,p.club)))throw Error('球员希望加盟更高级别球队');
 return {p,roster};
}
