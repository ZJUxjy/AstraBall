...
import {populationPlayers,populationPlayer,clubPlayers,ensurePopulation,setPlayerClub,currentAbility,promotePlayer} from './population.js';
...
function contractFor(s,p){const youth=p.unit==='youth',years=1+hash(`contract:${p.id}`)%3;return {club:p.club,start:s.date,end:dateOf(youth?p.birthYear+20:s.year+years-1,12,31),weeklyWage:youth?140:weeklyWage(s,p),kind:youth?'youth':'senior'};}
export function ensureEconomy(s){
 ...
 for(const p of populationPlayers(s))if(p.club)e.contracts[p.id]=contractFor(s,p);
 for(const team of footballTeams){const terms=annualTerms(s,team.id),bill=clubPlayers(s,team.id,{unit:'all'}).reduce((n,p)=>n+e.contracts[p.id].weeklyWage,0);terms.wageLimit=Math.max(terms.wageLimit,Math.ceil(bill*1.15/7)*7);const cash=terms.funding+terms.transferBudget;e.accounts[team.id]={cash,openingCash:cash,...terms,baseWageLimit:terms.wageLimit,baseLevel:clubLevel(s,team.id),spent:0,totals:zero(),season:zero(),history:[]};}
 return e;
}
export function wageBill(s,club){const e=ensureEconomy(s);return clubPlayers(s,club,{unit:'all'}).reduce((n,p)=>n+(e.contracts[p.id]?.weeklyWage||0),0);}
...
function ownPlayer(s,id){const p=populationPlayer(s,id);if(!s.manager||!p||p.club!==s.manager.clubId||p.retired)throw Error('只能管理本队球员合同');return p;}
...
export function transferQuote(s,id,buyer,years=3,{sellerApproved=false}={}){
 ...
 if(!p||p.retired||!['senior','free'].includes(p.unit))throw Error('球员不可签约');
 ...
 if(clubPlayers(s,buyer).length>=30)throw Error('一线队已满 30 人');
...
export function signPlayer(s,id,buyer=s.manager?.clubId,years=3,{automatic=false,sellerApproved=false}={}){
 ...
 if(seller){const oldDivision=Object.keys(s.members).find(d=>s.members[d].includes(seller)),newDivision=Object.keys(s.members).find(d=>s.members[d].includes(buyer));if(oldDivision!==newDivision&&s.discipline[`${oldDivision}/${id}`]){const discipline=s.discipline[`${oldDivision}/${id}`];s.discipline[`${newDivision}/${id}`]={yellow:0,ban:discipline.ban};delete s.discipline[`${oldDivision}/${id}`];}}
 move(e,s,'transfer',p,q);s.revision++;return q;
}
export function renewalQuote(s,id,years=3){
 ...
 if(bill>a.wageLimit||bonus>a.cash-reserve)throw Error('续约预算不足');
...
export function renewPlayer(s,id,years=3,{automatic=false}={}){
 ...
 const q=renewalQuote(s,id,years);record(e,p.club,'bonuses',q.bonus);e.contracts[id]={club:p.club,start:s.date,end:q.end,weeklyWage:q.weeklyWage,kind:p.unit==='youth'?'youth':'senior'};move(e,s,'renewal',p,q);s.revision++;return q;
}
...
function release(s,p,type,extra={}){const e=s.economy,club=p.club;p.lastClub=club;p.freeSince=s.date;setPlayerClub(s,p,null);p.unit='free';delete e.contracts[p.id];delete s.development?.plans[p.id];move(e,s,type,p,{from:club,...extra});}
...
export function syncContracts(s){const e=ensureEconomy(s);for(const [id,c] of Object.entries(e.contracts)){const p=populationPlayer(s,id);if(!p||p.retired||p.club!==c.club)delete e.contracts[id];}for(const p of populationPlayers(s))if(p.club&&!e.contracts[p.id])e.contracts[p.id]=contractFor(s,p);for(const p of populationPlayers(s))if(p.unit==='senior'&&e.contracts[p.id]?.kind==='youth'){const c=e.contracts[p.id];c.kind='senior';c.weeklyWage=weeklyWage(s,p);c.end=dateOf(s.year+2,12,31);}}
export function rolloverEconomy(s){
 ...
 for(const team of footballTeams){const a=e.accounts[team.id];a.history.push({year:e.year,...a.season,cash:a.cash});a.season=zero();const terms=annualTerms(s,team.id);a.baseWageLimit??=a.wageLimit;a.baseLevel??=clubLevel(s,team.id);terms.wageLimit=Math.max(terms.wageLimit,Math.round(a.baseWageLimit*wageAt(clubLevel(s,team.id))/wageAt(a.baseLevel)/7)*7);Object.assign(a,terms,{spent:0});record(e,team.id,'funding',a.funding);}
 ...
 const expired=Object.entries(e.contracts).filter(([,c])=>c.end<s.date).map(([id])=>populationPlayer(s,id));
...
function aiReview(s){
 const e=ensureEconomy(s);if(!isTransferWindow(s))return;
 const all=populationPlayers(s).filter(p=>['senior','free'].includes(p.unit)&&p.club!==s.manager?.clubId&&(!p.lastTransfer||daysBetween(p.lastTransfer,s.date)>=90)),ability=new Map(all.map(p=>[p.id,currentAbility(s,p)])),offered=new Set();
 ...
 for(const team of order){const roster=clubPlayers(s,team.id),level=clubLevel(s,team.id);if(roster.length>=27)continue;const depths=new Map();for(const p of roster){const depth=depths.get(p.position)||{count:0,low:100};depth.count++;depth.low=Math.min(depth.low,ability.get(p.id)??currentAbility(s,p));depths.set(p.position,depth);}
  const targets=candidates.filter(p=>p.club!==team.id&&!p.retired&&p.unit!=='youth'&&ability.get(p.id)<=level+11&&ability.get(p.id)>=level-16).map(p=>{const depth=depths.get(p.position)||{count:0,low:100},needed=depth.count<(p.position==='GK'?3:2),quality=ability.get(p.id),improvement=quality-depth.low;return {p,score:(needed?100:0)+(roster.length<24?60:0)+improvement,needed,improvement,quality};}).filter(x=>(x.needed||roster.length<24||x.improvement>5)&&x.quality>=level-16).sort((a,b)=>b.score-a.score||a.p.id.localeCompare(b.p.id));
...
export function advanceCareer(s,date){
 ...
 while(e.through<date){const end=[date,e.nextReview].sort()[0];advanceDevelopment(s,end);accrueEconomy(s,end);s.date=end;if(end===e.nextReview){aiReview(s);e.nextReview=addDays(end,7);}}
 s.date=date;
}
export function validateEconomy(s){
 ...
 for(const [id,c] of Object.entries(e.contracts)){const p=populationPlayer(s,id);if(!p||p.retired||!p.club||p.club!==c.club||!validDate(c.start)||!validDate(c.end)||c.start>c.end||!integer(c.weeklyWage)||c.weeklyWage%7!==0)throw Error('球员合同无效');}
 if(populationPlayers(s).some(p=>p.club&&!e.contracts[p.id]))throw Error('球员合同缺失');
