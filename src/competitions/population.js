...
const registryView=p=>p&&({...p,unit:p.registrationStatus==='loan'?'senior':p.registrationStatus||'senior',birthYear:p.birthYear??(p.ageReferenceDate?Number(p.ageReferenceDate.slice(0,4)):YEAR)-p.age});
export const populationPlayers=(s,{retired=false}={})=>(s.population?Object.values(s.population.players):registeredPlayers(s,{includeRetired:retired}).map(registryView)).filter(p=>retired||!p.retired);
export const populationPlayer=(s,id)=>s.population?s.population.players[id]:registryView(registeredPlayer(s,id));
...
export function setPlayerClub(s,p,club){
 if(s.playerRegistry&&!s.population){p.club=club;return;}
...
export function promotePlayer(s,id,{automatic=false}={}){
 const pop=ensurePopulation(s),p=populationPlayer(s,id);
 ...
 p.unit='senior';p.promotedYear=s.year;numberPlayer(s,p);if(s.population)event(pop,s.year,'promotion',p,p.club);else persistPlayer(s,p,{type:'promotion'});s.revision++;
 return p;
}
