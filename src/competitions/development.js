...
const originals=footballTeams.flatMap(t=>t.roster),byId=new Map(originals.map(p=>[p.id,p]));
...
export function ageOnDate(p,date){const year=Number(date.slice(0,4));return p.age+year-YEAR+daysBetween(dateOf(year,1,1),date)/daysBetween(dateOf(year,1,1),dateOf(year+1,1,1));}
function recordFor(d,p){
 return d.records[p.id]??={attributes:{...p.attributes},sharpness:p.sharpness,startRating:preciseRating(p),seasonYear:Number(d.through.slice(0,4)),seasonRating:preciseRating(p),seasonAttributes:{...p.attributes},minutes:0,appearances:0,seasonMinutes:0,seasonAppearances:0,weekMinutes:0,weekChallenge:0,dose:zeroDose(),healthyDays:0,history:[{date:d.through,age:Math.floor(ageOnDate(p,d.through)),ability:preciseRating(p),minutes:0}],annual:[]};
}
export function developedPlayer(s,p,date=s.date){
 const r=s.development?.records[p.id];
...
export function recordDevelopmentMatch(s,input,result,date){
 ...
 for(let side=0;side<2;side++){
  ...
  for(const line of result.teams[side].players||[]){
   ...
   const p=byId.get(line.id);if(!p)continue;
...
export function setPlayerTraining(s,id,{focus,load}){
 const p=byId.get(id);if(!s.manager||p?.club!==s.manager.clubId)throw Error('只能安排本队球员训练');
...
export function developmentReport(s,p){
 const original=byId.get(p.id);if(!original)return null;
...
export function validateDevelopment(d){
 if(!d)return;
 ...
 for(const [id,r] of Object.entries(d.records))if(!byId.has(id)||!r||!groups.every(g=>Number.isFinite(r.dose?.[g])&&r.dose[g]>=0)||!['minutes','appearances','seasonMinutes','seasonAppearances','weekMinutes','weekChallenge','healthyDays'].every(k=>Number.isFinite(r[k])&&r[k]>=0)||!Array.isArray(r.history)||!Array.isArray(r.annual)||!Object.values(ATTRIBUTE_GROUPS).every(g=>Object.keys(g.fields).every(k=>Number.isFinite(r.attributes?.[k])&&r.attributes[k]>=1&&r.attributes[k]<=99&&Number.isFinite(r.seasonAttributes?.[k]))))throw Error('球员成长属性无效');
}
