...
import {recruitmentContext,recruitmentOrder,lacksPlayingTime,loanDestinations} from '../src/competitions/recruitment.js';
import {registeredRoster,validatePlayerRegistry} from '../src/competitions/registry.js';
...
test('AI 招募不自动处置玩家所属球员',()=>{
 const {s,p}=signedTalent();s.manager={clubId:source.id};s.date='0318-06-30';advanceYouthPathways(s,s.date);
...
test('满员AI队可在确认强援后置换无出场替补，人数和位置底线保持',()=>{
 const {s,p}=signedTalent(),target=recruitmentOrder(recruitmentContext(s,'0318-06-30',id=>registeredRoster(s,id)))[0];
 ...
 const roster=registeredRoster(s,target.id);assert.equal(roster.length,40);assert.ok(roster.filter(q=>q.position==='GK').length>=2);assert.ok(roster.filter(q=>q.position==='CB').length>=3);
