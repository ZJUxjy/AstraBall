...
test('全年赛历覆盖十二个月；联赛、帝国杯和淘汰赛留有独立日期',()=>{
 ...
 const s=createSeason();assert.equal(s.fixtures.length,4057);assert.equal(pendingMatches(s).length,4057);assert.equal(s.fixtures.filter(m=>m.bye).length,0);assert.ok(s.qualificationSource.includes('创始'));assert.equal(s.date,'0318-01-01');assert.equal(nextDate(s),'0318-01-20');
});
...
test('完整赛季各队无撞期、休息至少三天；所有冠军、晋级与降级闭合',()=>{
 ...
 const dates=new Map();for(const m of s.fixtures.filter(m=>!m.bye)){assert.ok(m.score);for(const id of [m.home,m.away]){const prev=dates.get(id);if(prev)assert.ok(daysBetween(prev,m.date)>=3,`${id}: ${prev} / ${m.date}`);dates.set(id,m.date);}if(m.kind==='playoff'&&!m.neutral){const order=tableFor(s,m.division).map(r=>r.id);assert.ok(order.indexOf(m.home)<order.indexOf(m.away));}if(!['league','group','championship-group'].includes(m.kind))assert.ok([m.home,m.away].includes(m.winner));}
