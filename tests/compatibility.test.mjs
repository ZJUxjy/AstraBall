...
test('V6—V17 未完赛快照在新版本继续，事件与各自旧引擎逐字一致',()=>{
...
test('V17 历史执行路径保持 V12 已校准的比赛动作与统计',()=>{
 const old=LEGACY_ENGINES[12];
 for(let seed=0;seed<20;seed++){
  const options={...input,seed:9200000+seed,ai:[seed%2===0,true]};
  const a=LEGACY_ENGINES[17].createMatch(options),b=old.createMatch(options);a.version=17;while(stepMatch(a));while(old.stepMatch(b));
  const {version:av,...ar}=getResult(a),{version:bv,...br}=old.getResult(b);assert.notEqual(av,bv);assert.deepEqual(ar,br);
 }
});

test('V18 人工控制下的比赛计算仍与已校准V12逐事件一致',()=>{const old=LEGACY_ENGINES[12];for(let seed=0;seed<20;seed++){const options={...input,seed:9300000+seed,ai:[false,false]},a=createMatch(options),b=old.createMatch(options);while(stepMatch(a));while(old.stepMatch(b));const {version:av,...ar}=getResult(a),{version:bv,...br}=old.getResult(b);assert.notEqual(av,bv);assert.deepEqual(ar,br);}});
