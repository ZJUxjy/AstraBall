...
test('318—325年度帝国杯每四年举办，杯赛年4057场、其他年4026场，旧规则存档拒绝混入',()=>{
...
test('结构赛果贯通资格与升降级：三平级和两大陆封闭，矿业岛3/3/2交换，无赛程冲突',()=>{
 ...
 const r=settleSeason(s);assert.equal(r.champions.length,13);assert.equal(r.movements.length,16);assert.ok(r.movements.every(m=>m.from.startsWith('liberlin-league')&&m.to.startsWith('liberlin-league')));
