...
export function createMatch({home,away,seed=1,homeTactics={},awayTactics={},homeLineup,awayLineup,neutral=false,knockout=false,capture=true,plans=[]}={}){
 ...
 return {version:ENGINE_VERSION,seed,plans:structuredClone(plans).sort((a,b)=>a.minute-b.minute),teams,random,neutral,knockout,capture,events:[],elapsed:0,period:1,periodClock:0,periodEnd:47*60,status:'playing',kickoff,side:kickoff,x:40,y:34,holder:null,lastPass:null,sequence:0,actions:0,shootout:null};
}
...
function teamAbility(state,team,keys){return mean(outfield(team).map(p=>ability(state,team,p,keys)))*Math.sqrt(team.slots.length/11);}
...
function clock(s,seconds,active=true){const dt=Math.min(seconds,s.periodEnd-s.periodClock);s.elapsed+=dt;s.periodClock+=dt;if(active)s.teams[s.side].stats.possessionSeconds+=dt;for(const t of s.teams)for(const slot of t.slots){const line=t.lines[slot.id];line.seconds+=dt;t.stats.playerSeconds+=dt;line.condition=clamp(line.condition-exertion(player(t,slot.id),t.tactics,dt*(active?1:.35),slot.position),15,100);}}
...
function foul(s,defender){
 ...
 clock(s,24,false);s.lastPass=null;if(s.status!=='playing')return;
...
export function stepMatch(s){if(s.status!=='playing')return false;if(++s.actions>20000)throw Error('比赛动作数超过安全上限');
 ...
 if(s.periodClock>=s.periodEnd){emit(s,'periodEnd');
  ...
  if(continues){
   ...
   s.period++;s.periodClock=0;s.periodEnd=(s.period===2?48:16)*60;s.pending=null;
