...
function shot(s,shooter,{kind='open',assist=s.lastPass}={}){
 ...
 const pressure=clamp(teamAbility(s,defense,['marking','positioning','concentration'])/130,.1,.9);
 const xG=shotQuality({x:s.x,y:s.y,pressure,kind});
...
function playAction(s){
 ...
 if(s.x>75&&s.random.next()<TUNE.shoot){shot(s,actor);return;}
 if(s.random.next()<.11){
