...
export function createMatch({home,away,seed=1,homeTactics={},awayTactics={},homeLineup,awayLineup,neutral=false,knockout=false,capture=true,plans=[]}={}){
 ...
 for(const t of teams)for(const p of t.roster)t.lines[p.id].form=random.normal()*(100-p.personality.consistency)*.035;
 return {version:ENGINE_VERSION,seed,plans:structuredClone(plans).sort((a,b)=>a.minute-b.minute),teams,random,neutral,knockout,capture,events:[],elapsed:0,period:1,periodClock:0,periodEnd:47*60,status:'playing',side:random.int(0,1),x:40,y:34,holder:null,lastPass:null,sequence:0,actions:0,shootout:null};
}
...
function clock(s,seconds,active=true){const dt=Math.min(seconds,s.periodEnd-s.periodClock);s.elapsed+=dt;s.periodClock+=dt;if(active)s.teams[s.side].stats.possessionSeconds+=dt;for(const t of s.teams)for(const slot of t.slots){const line=t.lines[slot.id];line.seconds+=dt;line.condition=clamp(line.condition-exertion(player(t,slot.id),t.tactics,dt*(active?1:.35),slot.position),15,100);}}
...
function shot(s,shooter,{kind='open',assist=s.lastPass}={}){
 ...
 const fin=ability(s,attack,shooter,kind==='header'?['heading','jumping','bravery']:s.x<82?['longShots','technique','composure']:['finishing','composure']),gkSkill=ability(s,defense,gk,s.x>93?['oneOnOnes','reflexes','rushingOut']:['reflexes','handling','agility']);
...
function playAction(s){
 ...
 const receiver=choose(s,attack,crossing?'shoot':'pass',actor.id),forward=s.random.next()<TUNE.advance+effects.advance,back=!forward&&s.random.next()<.18;
 const through=forward&&s.x>45&&s.random.next()<.18+(attack.tactics.passing==='direct'?.1:0);
 const distance=forward?14+s.random.next()*10+(through?8:0):back?-12:0;
 const target=crossing?[s.random.int(88,97),s.random.int(25,43)]:[clamp(s.x+distance,10,97),clamp(s.y+(s.random.next()-.5)*28*effects.width,5,63)];
 const skill=ability(s,attack,actor,crossing?['crossing','technique','vision']:through?['longPassing','vision','decisions']:['passing','decisions','technique'])+ability(s,attack,receiver,['firstTouch','offBall'])*.15;
 const pressure=teamAbility(s,defense,['anticipation','positioning','workRate'])*1.15;
 const probability=clamp(TUNE.passBase+(skill-pressure)*.0024+effects.completion-(crossing?.14:forward?.045:0)+(s.side===0&&!s.neutral?TUNE.homeEdge:0),.4,.97);
 const offside=through&&s.random.next()<clamp(.022+effects.behind+(65-ability(s,attack,receiver,['anticipation','offBall','decisions']))*.001,.003,.13);
 const success=!offside&&s.random.next()<probability;attack.stats.passes++;attack.lines[actor.id].passes++;
 emit(s,'pass',{player:actor.id,receiver:receiver.id,defender:defender.id,success,from:[s.side===0?s.x:105-s.x,s.side===0?s.y:68-s.y],to:s.side===0?target:[105-target[0],68-target[1]],kind:crossing?'cross':through?'through':forward?'progressive':back?'back':'short',offside});
 ...
 else{if(offside){attack.stats.offsides++;emit(s,'offside',{player:receiver.id});clock(s,12,false);}else{defense.stats.interceptions++;defense.lines[defender.id].interceptions++;}takeBall(s,1-s.side,105-target[0],68-target[1],defender.id);}
}
export function stepMatch(s){if(s.status!=='playing')return false;if(++s.actions>20000)throw Error('比赛动作数超过安全上限');
 while(s.plans.length&&s.plans[0].minute<=(s.period-1)*45+Math.min(s.periodClock,45*60)/60){const plan=s.plans.shift();applyCommand(s,plan.command);}
 playAction(s);
 if(s.periodClock>=s.periodEnd){emit(s,'periodEnd');if(s.period===1){for(const t of s.teams)for(const slot of t.slots)t.lines[slot.id].condition=clamp(t.lines[slot.id].condition+player(t,slot.id).attributes.naturalFitness*.055,0,100);s.period=2;s.periodClock=0;s.periodEnd=48*60;takeBall(s,1-s.side,52);emit(s,'periodStart');}else{s.status='finished';emit(s,'fullTime');}}
...
export function applyCommand(s,command){
 ...
 if(command.type==='tactics'){
  ...
  if(tactics.formation!==t.tactics.formation){
   const remaining=[...t.slots];const slots=FORMATIONS[tactics.formation].map(position=>{
 ...
 const sameWindow=t.lastSubTime===s.elapsed;
 if(index<0||!incoming||!available(incoming)||t.used.has(incoming.id)||t.subs>=5||!sameWindow&&t.windows>=3)throw Error('换人不符合规则');
 const outgoing=t.slots[index];t.slots[index]={id:incoming.id,position:outgoing.position};
 t.used.add(incoming.id);t.subs++;if(!sameWindow)t.windows++;t.lastSubTime=s.elapsed;
